"""Streamlit application shell for the TMC Processor.

The shell owns bootstrap, mode selection, navigation, and compatibility
adapters.  Pure workflow decisions live in ``tmc_processor.application``;
reusable display boundaries live in ``tmc_processor.ui.components`` and
...
from tmc_processor.application.workflow import (
    CANONICAL_WORKFLOW_STAGES as APPLICATION_WORKFLOW_STAGES,
    canonical_workflow_stage as application_canonical_workflow_stage,
    batch_workflow_revisions as application_batch_workflow_revisions,
    single_workflow_revisions as application_single_workflow_revisions,
    synchronize_workflow as application_synchronize_workflow,
    workflow_stages_for_mode as application_workflow_stages_for_mode,
)
from tmc_processor.application.state import (
    confirm_batch_peak as application_confirm_batch_peak,
    confirm_single_peaks as application_confirm_single_peaks,
    clear_batch_review_state as application_clear_batch_review_state,
    clear_single_review_state as application_clear_single_review_state,
    get_batch_dispositions as application_get_batch_dispositions,
    get_confirmed_peaks as application_get_confirmed_peaks,
    set_batch_disposition as application_set_batch_disposition,
)
from tmc_processor.application.services import (
...
from tmc_processor.ui.workflows import render_workflow_stage
from tmc_processor.ui.workflow_context import WorkflowContext, WorkflowOperations
...
CANONICAL_WORKFLOW_STAGES = APPLICATION_WORKFLOW_STAGES
...
def _canonical_workflow_stage(value: object) -> str:
    ...
    if candidate in LEGACY_WORKFLOW_TAB_ALIASES:
        return LEGACY_WORKFLOW_TAB_ALIASES[candidate]
    return application_canonical_workflow_stage(candidate)
...
def workflow_stages_for_mode(mode: str) -> list[str]:
    return application_workflow_stages_for_mode(mode)
...
def _single_workflow_revisions(
    *,
    ...
    return application_single_workflow_revisions(
        source_bytes=source_bytes,
        source_revision=None if source_revision is _WORKFLOW_SOURCE_UNSET else source_revision,
        mapping=mapping,
...
def _batch_workflow_revisions(
    *,
    ...
    return application_batch_workflow_revisions(
        uploads=uploads,
        ...
        confirmed_peaks=confirmed_peaks,
        dispositions=dispositions or _batch_dispositions_from_state(),
        analysis_present=analysis_present,
    )


def _clear_single_review_state() -> None:
    application_clear_single_review_state(st.session_state)
    for key in (
...
def _clear_batch_review_state() -> None:
    application_clear_batch_review_state(st.session_state)
...
def _batch_dispositions_from_state() -> dict[str, dict[str, str]]:
    return application_get_batch_dispositions(st.session_state)
...
def _exclude_batch_file(folder_name: str, reason: str) -> bool:
    ...
    application_set_batch_disposition(st.session_state, str(folder_name), "excluded", item.exclusion_reason)
...
def _restore_batch_file(folder_name: str) -> bool:
    ...
    if item is None or getattr(item, "disposition", "") != "excluded":
        return False
    restore_batch_item(item)
    dispositions = application_get_batch_dispositions(st.session_state)
    dispositions.pop(str(folder_name), None)
    st.session_state[BATCH_DISPOSITION_STATE_KEY] = dispositions
    _mark_batch_export_stale_now()
    return True
...
def _sync_workflow_contract(
    ...
    synchronized = application_synchronize_workflow(
        previous_state,
        mode=mode,
        revisions=revisions,
        readiness=readiness,
    )
    transition = synchronized.transition
    _apply_workflow_transition(mode, transition)
    _store_workflow_state(synchronized.state)
    return transition
...
def _process_single_file_for_ui(
    *,
    ...
    if scheme == MOVEMENT_SCHEME_V2:
        ...
        return application_analyze_single_dry_run(
    ...
    return application_analyze_single(
        raw_sheets=raw_sheets,
        mapping=mapping,
...
def _confirm_single_peak_review(
    ...
    changed = application_confirm_single_peaks(
        st.session_state,
        f"{current['am_peak_start']}-{current['am_peak_end']}",
        f"{current['pm_peak_start']}-{current['pm_peak_end']}",
    )
...
def _confirm_batch_peak_review(folder_name: str, am_peak: str, pm_peak: str) -> bool:
    ...
    changed = application_confirm_batch_peak(st.session_state, folder_name, am_peak, pm_peak)
...
def _render_readiness_checklist(items: list[tuple[str, bool, str]]) -> None:
    render_readiness_checklist_component(items)
...
def _render_qc_status(qc: pd.DataFrame | None, label: str = "QC status") -> None:
    render_qc_summary_component(_qc_severity_counts(qc), label=label)
...
def _workflow_operations() -> WorkflowOperations:
    """Return the explicit shell operations exposed to workflow stages."""
    return WorkflowOperations(
        AM_WINDOW=AM_WINDOW,
        BATCH_CONFIRMED_PEAKS_STATE_KEY=BATCH_CONFIRMED_PEAKS_STATE_KEY,
        BATCH_CONFIRMED_PEAK_SOURCE_STATE_KEY=BATCH_CONFIRMED_PEAK_SOURCE_STATE_KEY,
        BATCH_DISPOSITION_STATE_KEY=BATCH_DISPOSITION_STATE_KEY,
        BATCH_DRAFT_PEAKS_STATE_KEY=BATCH_DRAFT_PEAKS_STATE_KEY,
        BATCH_EXCEL_TEMPLATE_EXPORT_MODE=BATCH_EXCEL_TEMPLATE_EXPORT_MODE,
        BATCH_PACKAGE_MIME=BATCH_PACKAGE_MIME,
        BATCH_SAFE_PNG_EXPORT_LABEL=BATCH_SAFE_PNG_EXPORT_LABEL,
        BATCH_SAFE_PNG_EXPORT_MODE=BATCH_SAFE_PNG_EXPORT_MODE,
        BATCH_V2_TEMPLATE_MODE_UNSUPPORTED_TH=BATCH_V2_TEMPLATE_MODE_UNSUPPORTED_TH,
        BytesIO=BytesIO,
        ...
        EXPORT_PREFERENCE_ADVANCED=EXPORT_PREFERENCE_ADVANCED,
        EXPORT_PREFERENCE_STANDARD=EXPORT_PREFERENCE_STANDARD,
        MAPPING_PRESET_MIME=MAPPING_PRESET_MIME,
        MAPPING_SCHEME_LOCK_CAPTION=MAPPING_SCHEME_LOCK_CAPTION,
        MAPPING_SOURCE_DEFAULT_PREVIEW=MAPPING_SOURCE_DEFAULT_PREVIEW,
        MAPPING_SOURCE_MAPPING_EXCEL=MAPPING_SOURCE_MAPPING_EXCEL,
        MAPPING_SOURCE_MAPPING_PRESET=MAPPING_SOURCE_MAPPING_PRESET,
        ...
        PM_WINDOW=PM_WINDOW,
        PNG_MIME=PNG_MIME,
        PROJECT_SESSION_MIME=PROJECT_SESSION_MIME,
        Path=Path,
        STANDARD_REPORT_EXPORT_MODE=STANDARD_REPORT_EXPORT_MODE,
        TEMPLATE_VERSION=TEMPLATE_VERSION,
        V2_EXCEL_TEMPLATE_MODE_BLOCK_MESSAGE=V2_EXCEL_TEMPLATE_MODE_BLOCK_MESSAGE,
        WORKFLOW_BATCH_MODE=WORKFLOW_BATCH_MODE,
        WORKFLOW_SINGLE_MODE=WORKFLOW_SINGLE_MODE,
        WorkflowReadiness=WorkflowReadiness,
        _apply_mapping_editor_widget_state=_apply_mapping_editor_widget_state,
        _apply_standard_batch_export_mode=_apply_standard_batch_export_mode,
        _apply_standard_single_export_mode=_apply_standard_single_export_mode,
        _basic_mapping_widget_state_exists=_basic_mapping_widget_state_exists,
        _batch_export_mode_options=_batch_export_mode_options,
        _batch_export_signature=_batch_export_signature,
        _batch_items_from_uploads=_batch_items_from_uploads,
        _batch_qc_preview_display_frame=_batch_qc_preview_display_frame,
        _batch_qc_rows_for_ui=_batch_qc_rows_for_ui,
        _batch_status_display_frame=_batch_status_display_frame,
        _batch_status_frame=_batch_status_frame,
        _batch_summary_counts=_batch_summary_counts,
        _batch_workflow_revisions=_batch_workflow_revisions,
        _build_session_from_state=_build_session_from_state,
        _bulk_accept_clean_batch_files=_bulk_accept_clean_batch_files,
        _clear_mapping_for_scheme_change=_clear_mapping_for_scheme_change,
        _coerce_export_mode=_coerce_export_mode,
        _coerce_setup_time=_coerce_setup_time,
        _confirm_batch_peak_review=_confirm_batch_peak_review,
        _confirm_single_peak_review=_confirm_single_peak_review,
        _confirmed_peaks_from_state=_confirmed_peaks_from_state,
        _current_mapping_scheme=_current_mapping_scheme,
        _default_batch_export_mode=_default_batch_export_mode,
        _default_single_export_mode=_default_single_export_mode,
        _display_status_label=_display_status_label,
        _exclude_batch_file=_exclude_batch_file,
        _existing_columns=_existing_columns,
        _export_single_file_for_ui=_export_single_file_for_ui,
        _flash_and_rerun=_flash_and_rerun,
        _format_display_columns=_format_display_columns,
        _hourly_interval_options=_hourly_interval_options,
        _hourly_movement_for_ui=_hourly_movement_for_ui,
        _interval_total_pcu=_interval_total_pcu,
        _is_v2_result=_is_v2_result,
        _is_v2_scheme=_is_v2_scheme,
        _mapping_aggregation_preview=_mapping_aggregation_preview,
        _mapping_editor_changed=_mapping_editor_changed,
        _mapping_editor_column_config=_mapping_editor_column_config,
        _mapping_editor_frame=_mapping_editor_frame,
        _mapping_editor_key=_mapping_editor_key,
        _mapping_from_editor_widget_state=_mapping_from_editor_widget_state,
        _mapping_issue_display=_mapping_issue_display,
        _mapping_preset_rows_frame=_mapping_preset_rows_frame,
        _mapping_rows_are_committed=_mapping_rows_are_committed,
        _mapping_source=_mapping_source,
        _mapping_workspace_counts=_mapping_workspace_counts,
        _mark_batch_export_stale_now=_mark_batch_export_stale_now,
        _merge_mapping_editor_result=_merge_mapping_editor_result,
        _movement_code_options_for_scheme=_movement_code_options_for_scheme,
        _pce_override_summary=_pce_override_summary,
        _peak_period_text=_peak_period_text,
        _peak_value_payload=_peak_value_payload,
        _peak_values_complete=_peak_values_complete,
        _process_single_file_for_ui=_process_single_file_for_ui,
        _qc_display_frame=_qc_display_frame,
        _qc_severity_counts=_qc_severity_counts,
        _record_workflow_state=_record_workflow_state,
        _render_action_hint=_render_action_hint,
        _render_alert=_render_alert,
        _render_basic_mapping_editor=_render_basic_mapping_editor,
        _render_download_button=_render_download_button,
        _render_effective_peak_export_confirmation=_render_effective_peak_export_confirmation,
        _render_empty_state=_render_empty_state,
        _render_excel_com_status=_render_excel_com_status,
        _render_hourly_pcu_line_chart=_render_hourly_pcu_line_chart,
        _render_mapping_scheme_status=_render_mapping_scheme_status,
        _render_metric_strip=_render_metric_strip,
        _render_pce_factor_editor=_render_pce_factor_editor,
        _render_peak_card=_render_peak_card,
        _render_qc_status=_render_qc_status,
        _render_readiness_checklist=_render_readiness_checklist,
        _render_section_header=_render_section_header,
        _render_status_chip=_render_status_chip,
        _render_template_audit_notes=_render_template_audit_notes,
        _restore_batch_file=_restore_batch_file,
        _scheme_select_label=_scheme_select_label,
        _selected_interval=_selected_interval,
        _set_current_mapping_scheme=_set_current_mapping_scheme,
        _set_mapping_source=_set_mapping_source,
        _single_effective_peak_state=_single_effective_peak_state,
        _single_export_mode_options=_single_export_mode_options,
        _single_file_mapping_issues=_single_file_mapping_issues,
        _single_file_processing_block_reason=_single_file_processing_block_reason,
        _single_workflow_revisions=_single_workflow_revisions,
        _stable_batch_confirmed_peaks=_stable_batch_confirmed_peaks,
        _standard_report_decision=_standard_report_decision,
        _sync_batch_analysis_metadata_from_state=_sync_batch_analysis_metadata_from_state,
        _sync_batch_dispositions_to_analysis=_sync_batch_dispositions_to_analysis,
        _sync_batch_workflow_from_state=_sync_batch_workflow_from_state,
        _sync_single_workflow_from_state=_sync_single_workflow_from_state,
        _sync_workflow_after_pce_editor=_sync_workflow_after_pce_editor,
        _thai_aggregation_message=_thai_aggregation_message,
        _thai_mapping_control_warning=_thai_mapping_control_warning,
        _time_from_text=_time_from_text,
        _use_excel_native_charts_for_export=_use_excel_native_charts_for_export,
        _use_template_layout_for_export=_use_template_layout_for_export,
        _v2_movement_code_reference_frame=_v2_movement_code_reference_frame,
        ...
        apply_mapping_preset_to_detected_sheets=apply_mapping_preset_to_detected_sheets,
        apply_saved_mapping_to_sheets=apply_saved_mapping_to_sheets,
        apply_single_export_mode_change=apply_single_export_mode_change,
        batch_inputs_ready=batch_inputs_ready,
        batch_package_filename=batch_package_filename,
        batch_review_state=batch_review_state,
        batch_selected_file_preview=batch_selected_file_preview,
        batch_zip_contents_preview=batch_zip_contents_preview,
        batch_zip_generation_block_reason=batch_zip_generation_block_reason,
        build_export_summary_text=build_export_summary_text,
        build_mapping_preset=build_mapping_preset,
        build_setup_for_processing=build_setup_for_processing,
        build_v2_movement_diagram_data=build_v2_movement_diagram_data,
        create_export_package_zip=create_export_package_zip,
        create_v2_generated_export_package_zip=create_v2_generated_export_package_zip,
        default_mapping_for_sheets=default_mapping_for_sheets,
        detect_mapping_preset_scheme=detect_mapping_preset_scheme,
        detect_raw_direction_sheet_names=detect_raw_direction_sheet_names,
        download_buffer=download_buffer,
        eligible_clean_batch_items=eligible_clean_batch_items,
        format_count=format_count,
        format_pcu=format_pcu,
        generate_four_leg_tmc_diagram=generate_four_leg_tmc_diagram,
        generated_timestamp_text=generated_timestamp_text,
        hashlib=hashlib,
        load_mapping_preset=load_mapping_preset,
        mapping_control_warnings=mapping_control_warnings,
        mapping_to_excel_bytes=mapping_to_excel_bytes,
        movement_aggregation_audit=movement_aggregation_audit,
        movement_aggregation_messages=movement_aggregation_messages,
        normalize_approach_movement_mapping=normalize_approach_movement_mapping,
        pd=pd,
        probe_excel_com=probe_excel_com,
        read_mapping_excel_with_metadata=read_mapping_excel_with_metadata,
        report_chart_pngs=report_chart_pngs,
        reviewed_peak_values_complete=reviewed_peak_values_complete,
        safe_mapping_preset_filename=safe_mapping_preset_filename,
        safe_output_stem=safe_output_stem,
        safe_package_filename=safe_package_filename,
        safe_project_session_filename=safe_project_session_filename,
        safe_workbook_filename=safe_workbook_filename,
        serialize_mapping_preset=serialize_mapping_preset,
        session_to_json_bytes=session_to_json_bytes,
        set_active_tab=set_active_tab,
        validate_mapping_scheme=validate_mapping_scheme,
        vehicle_composition_report=vehicle_composition_report,
        warnings=warnings,
    )
    ...
    if not is_single_file_mode:
        render_workflow_stage(
            ...
            context=WorkflowContext(
                ...
                previews=previews,
                selected_pce_factors=selected_pce_factors,
                setup=setup,
                show_u_turn=show_u_turn,
                single_export_options=single_export_options,
                south_label=south_label,
                south_road=south_road,
                survey_date_text=survey_date_text,
                survey_period=survey_period,
    ...
    if is_single_file_mode:
        render_workflow_stage(
            ...
            context=WorkflowContext(
                ...
                batch_uploads=batch_uploads,
                caption_text=caption_text,
                confirmed_am_end=confirmed_am_end,
                confirmed_am_start=confirmed_am_start,
                confirmed_pm_end=confirmed_pm_end,
                confirmed_pm_start=confirmed_pm_start,
                detected_sheet_names=detected_sheet_names,
                east_label=east_label,
                east_road=east_road,
                excel_com_status=excel_com_status,
                ...
                north_road=north_road,
                parsed_details=parsed_details,
                pce_results_stale=pce_results_stale,
                peak_mode=peak_mode,
                peak_windows=peak_windows,
                preview_summary=preview_summary,
                previews=previews,
